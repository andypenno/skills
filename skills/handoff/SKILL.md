...
name: handoff
description: |-
  Trigger when work needs to continue in a different session or by a different agent - the user asks for a prompt for the next agent, a handoff, a summary of where things are up to, or is stopping mid-task. Also when the user wants a prompt for a fresh session or another agent to test or verify this work and report back ("give me a prompt to test it", "a prompt with what you'll need back"), or the next step needs access this session lacks (a running server, another repo, another machine). For a contract with an agent that owns a different system, use /agent-brief.
  Keywords: handoff, hand over, prompt for the next agent, next session, where were we, where is it up to, pick this up later, self-contained prompt, pass this on, brief the next agent, prompt to test it, prompt for a new session, report back
---
