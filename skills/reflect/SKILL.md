...
| Backup mirror | `~/.claude-session-backups/projects` | `REFLECT_BACKUP_DIR` (a mirror moved with the backup script's `CLAUDE_SESSION_BACKUP_DIR` lives at `$CLAUDE_SESSION_BACKUP_DIR/projects`) |
...
The script ships next to this `SKILL.md`. When the harness names this skill's base directory on load, use `<base>/extract-corpus.sh`. Otherwise locate it rather than hardcoding a path - it lands somewhere different under a plugin install, a `~/.claude/skills` copy, and a `~/.agents/skills` symlink, and the plugin cache keeps one copy per past version, so take the newest:

```bash
EXTRACT="$(fd -H -L -t f '^extract-corpus\.sh$' ~/.claude/plugins ~/.claude/skills ~/.agents/skills 2>/dev/null -X ls -t | head -1)"
...
1. **Mine (parallel, perspective-diverse).** Several agents each read the *whole* corpus (pass the chunk paths from the manifest) but hunt **one** lens apiece, so none gets distracted. Rotate/refresh the lenses between runs; a good spread:
   ...
   Each finding must cite **2 to 4 verbatim quotes** copied from the corpus, plus the distinct sessions/projects it spans and its recency by session date.
...
3. **Verify (parallel, one per top candidate).** Each agent greps the *real* `.jsonl` transcripts (`rg`/`jq` over `~/.claude/projects/<slug>/*.jsonl` and the backup mirror) to confirm the pattern recurs in 2+ sessions. It excludes earlier `reflect` sessions (any whose user text invokes reflect) and their `subagents/`, which quote the originals back and inflate every count. It dates each occurrence against `git log --date=iso` of the instruction file that covers the topic now, or its source template, since a complaint dated before the commit that addressed it caused that commit and is no evidence it failed. A file with no history leaves the occurrence marked undated, never dropped. It also captures the assistant action that triggered the correction (the root cause, not the symptom), and **reads every instruction file discovered in Step 4** (the repo's `CLAUDE.md`/`AGENTS.md` set - root, `.claude/`, and per-directory - plus `~/.claude/CLAUDE.md` and any `CLAUDE.local.md`) to confirm it isn't already covered. Be adversarial: drop one-offs, noise, and already-covered items. Mining can hallucinate - never trust a pattern you have not grounded in a real quote.
4. **Synthesize.** One agent, handed every verdict whole (side findings a verifier raised beyond its own candidate included), reads the current target files and produces (a) a ranked friction **report** with evidence and (b) **proposed edits**, each tagged with which file it belongs in (Step 4). Match each file's existing voice; do not duplicate rules already present; prefer fewer, sharper rules.
...
- **No silent caps.** If only the top N candidates get verified, the report names every one left unverified.
- **Spawn prompts.** Write each agent's prompt to `/agent-authoring`'s "Writing a spawn prompt".
...
```bash
fd -HI -E node_modules -E worktrees -i '^(claude|agents)(\.local)?\.md$' "<repo-root>"   # both conventions · root + .claude/ + nested
```
